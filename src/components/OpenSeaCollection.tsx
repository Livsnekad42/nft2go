...
const OpenSeaCollection = observer(
    class OpenSeaCollection extends React.Component<IRouteComponentProps<any>, IState> {
        ...
        constructor(props:any) {
            ...
            this.handleLoadOpenSeaNFTs = this.handleLoadOpenSeaNFTs.bind(this);
        ...
        async addFavoriteHandler(event: any, nftId: any, collection: any) {
                await addFavoriteOpensea({nftId, collectionId: collection});
            ...
            openseaStore.getCollNFTs(this.currID, 1, 10)
        }

        async deleteFavoriteHandler(event: any, nftId: any, collection: any) {
                await deleteFavoriteOpensea({nftId, collectionId: collection});
            openseaStore.openseaNFTs.nfts.map((nft: any) => {
                    if (nft.id === nftId) {
                        runInAction(() => {
        ...
        async handleLoadOpenSeaNFTs(page: any) {
            globalStore.setLoader(true);
            if (this.state.load) {
                return
            }
            await this.setState({load: true})
            let len = openseaStore.openseaCollNFTs.nfts.length;
            await openseaStore.getCollNFTs(this.currID, page, 10, this.state.query, this.state.typeContent, this.state.price.min, this.state.price.max, false)
            if (len === openseaStore.openseaCollNFTs.nfts.length) {
        ...
        async goSearch(event: any) {
            event.preventDefault();
            await openseaStore.getCollNFTs(this.currID, 1, 10, this.state.query);
        }

        componentDidMount() {
            openseaStore.getSingleCollection(this.currID);
            openseaStore.getCollNFTs(this.currID);
        ...
        render() {
            ...
            return (
                <>
                    ...
                    <main>
                        <img src={openseaStore.openseaCollection && openseaStore.openseaCollection.banner_image_url ? openseaStore.openseaCollection.banner_image_url : ""} style={{ width:"100vw", maxHeight:"190px", height:"100%", position:"absolute", zIndex:10}} alt=""/>
                        <div className="container-ext">
                            <div className="container">
                                <div id="nft-top-wrapper" className="d-flex flex-column justify-content-center align-items-center" style={{margin: "0 auto"}}>
                                    ...
                                    <img src={openseaStore.openseaCollection.image_url} className="img-fluid" style={{zIndex:30}}/>
                                    <h1>{openseaStore.openseaCollection.name}</h1>
                                    <p style={{width:"100%", wordWrap: "break-word", textAlign:"center"}}>Создано {openseaStore.openseaCollection.id}</p>
                                    ...
                                    <p id="coll-description">
                                        {openseaStore.openseaCollection.description}
                                    </p>
                                </div>
                            </div>
                            <div className="row">
                                    ...
                                    <div className="product-list">
                                        <div className="row justify-content-center align-items-center" id="wrapper-cart-list" style={{position:"relative" }}>
                                            {this.state.view === 'nft' ? <InfiniteScroll
                                                pageStart={1}
                                                loadMore={this.handleLoadOpenSeaNFTs}
                                                                        ...
                                                                        <div className="cart-cell-container-icon">
                                                                            {isLogged() ? !!nft.isFavorite ?
                                                                                <button className="share-btn-cart"
                                                                                        onClick={(e) => this.deleteFavoriteHandler(e, nft.id, nft.collection.id)}>
                                                                                ...
                                                                                <button className="share-btn-cart"
                                                                                        onClick={(e) => this.addFavoriteHandler(e, nft.id, nft.collection.id)}>
                                                                                    <svg width="19" height="25" viewBox="0 0 19 25" fill="none"
                                                                                         xmlns="http://www.w3.org/2000/svg">
                                                                                        <path
                                                                                            d="M0 25V2.34375C0 1.04932 1.04932 0 2.34375 0H16.4062C17.7007 0 18.75 1.04932 18.75 2.34375V25L9.375 19.5312L0 25Z"
                                                                        ...
                                                                        <div
                                                                            ...
                                                                            <div className="d-flex flex-column justify-content-between">
                                                                                ...
                                                                                <div className="itself-name-collection"><Link
                                                                                    to={"/nfts/" + nft.id}>{nft.name ? nft.name.slice(0, 7) + ".." : ""}</Link></div>
                                                                            </div>
                                                                            <div className="d-flex flex-column justify-content-between">
                                                        ...
                                                        return (
                                                            <div className="col-12">
                                                                <div className="single-cart-cell-list d-flex justify-content-center align-items-center">
                                                                    <img className="img-fluid" src={nft.image_url}/>
                                                                    <span>{nft.name.slice(0, 7) + ".."}</span>
                                                                    <p>{nft.description.slice(0, 7) + ".."}</p>
                                                                </div>
                                                            </div>
                                                        )
                                                    })}
